...
public class CharNBoolean {
	...
	public static void main(String[] args) {
		//char형(기본 자료형):하나의 문자를 저장할 수 있는 자료형(2byte),0부터 ~2^16-1까지 저장.
		//     값 저장시 하나의 문자를 ''(single quotation)으로 감싼다.
		//     ""(double quotation)으로 감싸면 무조건 String형(참조형)
		//char ch1 ="가";//[x]"가"는 문자열 즉 String타입
		//char ch1 ='가나';//[x]문자는 반드시 하나여야 한다.
		char ch1 ='가';//[o]'가'는 char형 상수
		
		char ch2='A';
		int num1=2;
		System.out.println(ch2 + num1);//[o] 'A'+2 = 65(A의 아스키 코드값)+2=67
		...
		System.out.println("num2를 char형으로 형변환:"+(char)num2);
		...
		System.out.println("십진수 44032(코드값)을 문자로 변환:"+(char)44032);
		
		char ch4 = '1';//숫자 문자 '1' 저장. 숫자 문자 '1'는 코드값이 49
		char ch4_1 = 49;//코드값(십진수) 저장
		//위의 두 변수에는 같은 데이타(문자)가 저장됨 즉 문자 '1'
		System.out.println("ch4="+ch4+",ch4_1="+ch4_1);
		//규칙2]char형에 값(데이타) 저장 
		//      1.''으로 감싸서 하나의 문자를 저장하거나 
		//      2.십진수 혹은 16진수로 저장하거나 
		//      3.\\u16진수형태의 값을 저장 할수 있다. 
		//      십진수로 저장시 그 숫자는 아스키 코드 혹은 유니코드값이 된다.
		char ch5='가';
		char ch5_1=44032;//44032는 16진수로 AC00=A(10)*16^3+C(12)*16^2+0+0=40960+3072
		char ch5_2=0xAC00;
		char ch5_3='\uAC00';//''(싱글쿼테이션)안에서 AC00이 유니코드라는 것을 나타내기 위해서 \\u를 추가한다.
