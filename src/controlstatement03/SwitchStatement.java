...
public class SwitchStatement {
	//switch문:if문 처럼 조건에 따라서 분기하는 제어문
	//형식:
	/*  
	...
	public static void main(String[] args) throws IOException {
		...
		if(remain == 0) System.out.println("나머지는 0");
		else if(remain == 1) System.out.println("나머지는 1");
		else System.out.println("나머지는 2");
		System.out.println("[switch문으로 나머지 값 판단]");
		switch (remain) {
			case 0:
		...
		//※switch문에는 정수식 즉 정수식의 실행 결과값이  byte/short/int/char/String타입/Enum타입이 와야함
	    // boolean값은 불가 즉 비교식이나 논리식 사용할 수 없다.
		//switch(inputNumber % 3 ==0) {}//[x] 비교식 불가
		//switch(inputNumber % 3 ==0 || inputNumber >=0) {}//[x] 논리식 불가
		byte b=5;
		switch(b) {//변수가 와도 됨(단,타입은 byte/short/char/int형/String형/Enum 만 가능)
			case 1:
				System.out.println("b에 저장된 값 1");
			/*
			case로 분기시 값 중복 불가(duplicate case에러)
			case 2:
		...
		long ln = 5;
		//switch(ln) {}//[x]long은 정수형이지만 불가
		//switch(ln % 3) {}//[x]정수식이지만 연산 결과가 long형이니까
		...
		switch(inputNumber) {
			...
			case 300:
				...
				break;
			default:System.out.println("위 사례에 해당하는 값이 없습니다");	
		}
