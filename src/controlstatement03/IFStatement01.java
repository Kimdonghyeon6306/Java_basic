...
public class IFStatement01 {
	...
	public static void main(String[] args) throws IOException {
		...
		*/
		...
		if(num1 % 2 != 0);
		//아래 블락{}는 if문과 전혀 관련없이 무조건 실행된다. 즉 위의 IF문에 연결된 IF블락이 아니다
		{
			System.out.printf("%d는 홀수%n",num1);
		}
		/*3]
		...
		*/	
		...
		/*
		System.out.println("사용자가 입력한 값(아스키 코드값):"+code);
		System.out.println("사용자가 입력한 값(문자):"+(char)code);
		//사용자가 입력한 문자가 숫자인지 아닌지를 판단하자
		//1]아스키 코드값을 알때
		boolean isNumber = code >=48 && code <=57;
		if(isNumber) System.out.println("입력한 문자는 숫자다");
		if(!isNumber) System.out.println("입력한 문자는 숫자가 아니다");
		//2]아스키 코드값을 모를때
		
		isNumber = code >='0' && code <='9';
		if(isNumber) System.out.println("입력한 문자는 숫자다");
		if(!isNumber) System.out.println("입력한 문자는 숫자가 아니다");
		*/
		/*
		...
		*/
		boolean isNumberOrAlphabet = code >='0' && code <='9' || 
				           code >='A' && code <='Z' || 
				           code >='a' && code <='z';
		...
		/*
		char character=(char)System.in.read();//\r을 읽는다
		System.out.println("입력한 문자:"+(int)character);
		character=(char)System.in.read();//\n을 읽는다
		System.out.println("입력한 문자:"+(int)character);*/
		//방법1)엔터값 읽어서 미 사용
		//System.in.read();//\r을 읽는다
		//System.in.read();//\n을 읽는다
		//방법2)
		System.in.skip(2);
		char character=(char)System.in.read();//사용자로 부터 입력 받기
		System.out.println("입력한 문자:"+character);
		/*
		...
		*/
		...
		boolean isMultiple = (character-'0') % 2 == 0;
